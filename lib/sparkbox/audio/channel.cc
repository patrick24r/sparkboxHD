...
Status Channel::SkipToTimeMicroseconds(size_t microseconds) {
  ...
  size_t sample_index = microseconds * audio_source_->sample_rate_hz_ / 1000000;
...
Status Channel::GetNextSamples(std::span<int16_t> samples, bool is_mono,
  ...
  if (filter_.denominator == 0 ||
      (sample_rate != audio_source_->sample_rate_hz_ * filter_.numerator /
                          filter_.denominator)) {
    filter_ = Resampler::GetResampleFilter(audio_source_->sample_rate_hz_,
                                           sample_rate);
  }
  ...
  for (size_t sample_filled_idx = 0; sample_filled_idx < samples.size();) {
    ...
    size_t source_samples_to_fill =
        ...
        audio_source_->number_of_channels_ / filter_.numerator;
    ...
    if (source_samples_to_fill % audio_source_->number_of_channels_ != 0) {
    ...
    source_samples_to_fill =
        std::max(source_samples_to_fill,
                 static_cast<size_t>(audio_source_->number_of_channels_));

    // Update the new number of samples to actually fill
    samples_to_fill =
        (source_samples_to_fill / audio_source_->number_of_channels_) *
    ...
    if (audio_source_->bytes_per_sample_ == 1) {
      Resampler::FormattedSamples<int8_t> samples_in_format = {
          .num_channels = audio_source_->number_of_channels_,
          .samples = std::span<int8_t>(
              reinterpret_cast<int8_t*>(audio_source_->bytes() +
                                        next_source_sample_index_),
              source_samples_to_fill)};
      Resampler::ResampleNextBlock<int8_t, int16_t>(filter_, samples_in_format,
                                                    samples_out_format);
    } else if (audio_source_->bytes_per_sample_ == 2) {
      Resampler::FormattedSamples<int16_t> samples_in_format = {
          .num_channels = audio_source_->number_of_channels_,
          .samples = std::span<int16_t>(
              reinterpret_cast<int16_t*>(audio_source_->bytes() +
                                         next_source_sample_index_),
              source_samples_to_fill)};
      Resampler::ResampleNextBlock<int16_t, int16_t>(filter_, samples_in_format,
                                                     samples_out_format);
    } else if (audio_source_->bytes_per_sample_ == 4) {
      Resampler::FormattedSamples<int32_t> samples_in_format = {
          .num_channels = audio_source_->number_of_channels_,
          .samples = std::span<int32_t>(
              reinterpret_cast<int32_t*>(audio_source_->bytes() +
                                         next_source_sample_index_),
              source_samples_to_fill)};
      Resampler::ResampleNextBlock<int32_t, int16_t>(filter_, samples_in_format,
                                                     samples_out_format);
    }

...
void FilesystemManager::RunTest() {
  ...
  if (status != Status::kOk) {
    SP_LOG_ERROR("Create directory failed with status: %d",
  ...
  // Write test
  status = driver_.Open(file, testFile,
                        FilesystemDriver::kWrite | FilesystemDriver::kCreate);
  if (status != Status::kOk) {
    SP_LOG_ERROR("Write test open failed with status: %d",
                 static_cast<int>(status));
    return;
  }

  status = driver_.Write(file, reinterpret_cast<const char*>(testText.data()),
                         testText.length(), byte_result);
  driver_.Close(file);
  if (status != Status::kOk) {
    SP_LOG_ERROR("Write test write failed with status: %d",
                 static_cast<int>(status));
    return;
  }
  ...
  if (status != Status::kOk) {
    SP_LOG_ERROR("Read test open failed with status: %d",
                 static_cast<int>(status));
    return;
  }
  ...
  if (status != Status::kOk) {
    SP_LOG_ERROR("Remove file failed with status: %d",
                 static_cast<int>(status));
    return;
  }
  ...
  if (status != Status::kOk) {
    SP_LOG_ERROR("Remove directory failed with status: %d",
                 static_cast<int>(status));
    return;
  }
